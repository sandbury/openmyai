//这段代码的主要功能是支持浏览器的模块预加载。
// 定义一个对象属性的简化方法
var yr = Object.defineProperty;
// 定义一个方法，用于设置对象的属性，如果属性存在则更新，否则添加属性
...
(function () {
    ...
    function t(o) {
        if (o.ep) return;// 防止重复处理
        o.ep = !0;// 标记为已处理
        const i = n(o);// 获取请求参数
        fetch(o.href, i)// 发送请求以预加载模块
    }
})();
...
// 遍历数组 r 并执行每个元素（假设元素是函数）
function ot(r) {
    r.forEach(rr)// 使用 rr 执行
}
...
// 比较两个值 r 和 e，处理 NaN 和对象类型
function xe(r, e) {
    return r != r ? e == e : r !== e || r && typeof r == "object" || typeof r == "function"
}

let Vt;// 用于存储临时创建的链接元素
// 检查两个 URL 是否相同，处理绝对路径和相对路径
function or(r, e) {
...
// 检查对象 r 是否为空对象
function _r(r) {
    return Object.keys(r).length === 0 // 检查属性键的数量
}

// 处理一些逻辑，如果 r 存在，则执行特定操作
function Gt(r, e, n, t) {
    if (r) {
        const o = ir(r, e, n, t);// 假设 ir 是定义的某个函数
        return r[0](o)// 执行 r 数组的第一个函数，并传入参数 o
    }
}

//这段代码提供了一系列用于 DOM 操作的函数，可以用于创建、插入、删除元素和处理事件。
// 根据传入的参数处理 ctx 和调用 r[1] 函数
function ir(r, e, n, t) {
    return r[1] && t ? vr(n.ctx.slice(), r[1](t(e))) : n.ctx// 如果 r[1] 存在且 t 存在，调用并返回结果
}

// 根据传入的参数处理 dirty 状态
function Ut(r, e, n, t) {
    if (r[2] && t) {
        const o = r[2](t(n));// 调用 r[2] 函数
        if (e.dirty === void 0) return o; // 如果 e.dirty 未定义，返回 o
        if (typeof o == "object") {
            const i = [], s = Math.max(e.dirty.length, o.length);
            for (let l = 0; l < s; l += 1) i[l] = e.dirty[l] | o[l];// 处理 dirty 数组
            return i
        }
        return e.dirty | o// 返回更新后的 dirty
    }
    return e.dirty// 返回原有的 dirty
}

// 更新渲染的状态
function Zt(r, e, n, t, o, i) {
    if (o) {
        const s = ir(e, n, t, i);// 获取处理后的 ctx
        r.p(s, o)// 调用 r 的 p 方法
    }
}

// 如果 ctx 长度超过 32，返回填充数组
function Wt(r) {
    if (r.ctx.length > 32) {
        const e = [], n = r.ctx.length / 32;
        for (let t = 0; t < n; t++) e[t] = -1;// 初始化填充数组
        return e
    }
    return -1
}
...
// 在 r 中插入元素 e，位置在 n 前
function U(r, e, n) {
    r.insertBefore(e, n || null)// 默认在末尾插入
}
...
//这段代码处理组件的状态和生命周期管理，允许在组件挂载时执行特定的操作
// 定义一个包装器函数，管理组件的状态更新
function wr(r) {
    let e;// 用于存储参数
    return {
        p(...n) {// 定义 p 方法，接受多个参数
            e = n, e.forEach(t => r.push(t))// 将参数添加到 r 中
        }, r() {
            e.forEach(n => r.splice(r.indexOf(n), 1))// 从 r 中移除之前添加的参数
        }
    }
}
...
// 更新文本节点的数据
function Ue(r, e) {
    e = "" + e, r.data !== e && (r.data = e)// 只有在数据不同的情况下才更新
}

// 设置输入元素的值
function bt(r, e) {
    r.value = e ?? ""// 如果 e 为 null 或 undefined，设置为空字符串
}
...
const St = [], At = [];
let Ot = [];// 存储待处理的任务
const sn = [], $r = Promise.resolve();// 存储待执行的函数,创建一个解决的 Promise
let ln = !1;// 标记是否已启动
// 启动状态更新流程
function Sr() {
    ln || (ln = !0, $r.then(lr))// 如果未启动，则标记并调用 lr
}
...
//这段代码主要关注于组件的更新逻辑，包括生命周期管理、状态更新、任务调度等。

const rn = new Set;// 用于跟踪已调用的任务
let $t = 0;// 当前任务索引
// 主循环，处理组件的更新和状态管理
function lr() {
    if ($t !== 0) return;// 如果当前任务已在进行，直接返回
    const r = Dt;// 获取当前组件
    do {
        try {// 执行所有待处理的组件更新
            for (; $t < St.length;) {
        ...
        // 处理完成后，清空待处理组件
        for (qt(null), St.length = 0, $t = 0; At.length;) At.pop()();// 执行待处理的函数
        for (let e = 0; e < Ot.length; e += 1) {
            const n = Ot[e];
            rn.has(n) || (rn.add(n), n())// 调用新的任务
        }
        Ot.length = 0// 清空任务列表
    } while (St.length);// 继续循环直到没有待处理组件
    // 执行所有待执行的函数
    for (; sn.length;) sn.pop()();
    ln = !1, rn.clear(), qt(r)// 重置状态
}

// 更新组件的渲染
function Or(r) {
    if (r.fragment !== null) {
        r.update(), ot(r.before_update);// 更新组件和前置操作
        const e = r.dirty;// 获取脏标记
        r.dirty = [-1], r.fragment && r.fragment.p(r.ctx, e), r.after_update.forEach(an)// 处理渲染
    }
}
...
function Lr(r) {
    const e = [], n = [];
    Ot.forEach(t => r.indexOf(t) === -1 ? e.push(t) : n.push(t)), n.forEach(t => t()), Ot = e// 分离任务,执行已存在的任务并更新任务列表
}

const zt = new Set;// 用于跟踪待处理的对象
let yt;// 当前对象
// 初始化对象的状态
function Ne() {
    yt = {r: 0, c: [], p: yt}// 创建新的状态对象
}

// 退出当前状态
function Qe() {
    yt.r || ot(yt.c), yt = yt.p// 如果未被引用，退出当前状态
}

// 初始化对象的效果
function M(r, e) {
    r && r.i && (zt.delete(r), r.i(e))// 调用初始化函数
}

// 管理对象的效果和销毁
function I(r, e, n, t) {
    if (r && r.o) {
        if (zt.has(r)) return;// 如果已处理，返回
        zt.add(r), yt.c.push(() => {
            zt.delete(r), t && (n && r.d(1), t())// 在销毁时调用
        }), r.o(e)// 调用效果函数
    } else t && t()
}
...
function on(r, e, n) {
    const t = r.$$.props[e];
    t !== void 0 && (r.$$.bound[t] = n, n(r.$$.ctx[t]))// 绑定并更新属性
}
...
// 将组件添加到 DOM
function F(r, e, n) {
    const {fragment: t, after_update: o} = r.$$;// 获取组件片段和更新后函数
    t && t.m(e, n), an(() => {
        const i = r.$$.on_mount.map(rr).filter(dt);// 处理挂载事件
        r.$$.on_destroy ? r.$$.on_destroy.push(...i) : ot(i), r.$$.on_mount = []// 清空挂载数组
    }), o.forEach(an)// 处理更新后事件
}

//这部分代码涉及了 Svelte 组件的创建、更新、销毁以及事件管理。它展示了如何通过状态管理和生命周期函数来控制组件行为.
// 销毁组件，清理其相关状态
function Y(r, e) {
    const n = r.$$;// 获取组件的状态对象
    n.fragment !== null && (// 如果存在片段
        Lr(n.after_update),// 执行更新后的操作
            ot(n.on_destroy),// 执行销毁操作
        n.fragment && n.fragment.d(e),// 从 DOM 中移除片段
            n.on_destroy = n.fragment = null, n.ctx = [])// 清理状态,重置上下文
}

// 更新组件的脏状态
function Tr(r, e) {
    r.$$.dirty[0] === -1 && (// 如果脏标记未初始化
        St.push(r),// 将组件推入更新队列
            Sr(),// 启动更新
            r.$$.dirty.fill(0)),// 重置脏标记
        r.$$.dirty[e / 31 | 0] |= 1 << e % 31// 更新脏状态
}
...
function $e(r, e, n, t, o, i, s, l = [-1]) {
    ...
    s && s(a.root);// 执行根节点相关操作
    let d = !1;// 标志变量
    if (a.ctx = n ? n(r, e.props || {}, (g, k, ...x) => { // 如果存在上下文
        const D = x.length ? x[0] : k;// 获取第一个参数或使用 k
        return a.ctx && o(a.ctx[g], a.ctx[g] = D) && (!a.skip_bound && a.bound[g] && a.bound[g](D), d && Tr(r, g)), k
    }) : [], // 更新上下文
        a.update(), d = !0, ot(a.before_update),// 执行更新
        a.fragment = t ? t(a.ctx) : !1, e.target) {// 如果存在目标
        if (e.hydrate) { // 如果需要水合
            const g = kr(e.target);// 获取子节点
            a.fragment && a.fragment.l(g), g.forEach(G)// 处理片段
        } else a.fragment && a.fragment.c();// 创建片段
        e.intro && M(r.$$.fragment),// 如果需要引入效果
            F(r, e.target, e.anchor),// 将组件添加到目标节点
            lr()// 启动主循环
    }
    qt(c)// 重置当前组件
}
...
class Se {
    ...
    // 销毁组件
    $destroy() {
        Y(this, 1), this.$destroy = se// 调用销毁方法
    }

    // 事件监听
    $on(e, n) {
        if (!dt(n)) return se;// 如果 n 不是函数，返回空操作
        const t = this.$$.callbacks[e] || (this.$$.callbacks[e] = []); // 获取回调
        return t.push(n), () => {
            const o = t.indexOf(n);
            o !== -1 && t.splice(o, 1)// 移除事件监听
        }
    }

    // 设置组件属性
    $set(e) {
        this.$$set && !_r(e) && (this.$$.skip_bound = !0, this.$$set(e), this.$$.skip_bound = !1) // 更新状态
    }
}

// 声明版本常量
const Er = "4";
typeof window < "u" && (window.__svelte || (window.__svelte = {v: new Set})).v.add(Er);// 注册 Svelte 版本
// 切片更新函数
function $n(r, e, n) {
    const t = r.slice();// 复制数组
    return t[4] = e[n], t[6] = n, t// 更新特定索引
}
...
function Sn(r) {
    ...
    return {
        c() {// 创建元素
            e = E("span"), e.innerHTML = "", v(e, "class", "inline")// 设置类名
        }, m(n, t) {// 挂载元素
            U(n, e, t)// 将元素插入到目标
        }, d(n) {// 销毁元素
            n && G(e)// 从 DOM 中移除
    ...
    return {
        c() { // 创建 DOM 元素
            e = E("span"),// 创建 span 元素
                t = we(n),// 设置内容为 n
                i = Z(),// 创建空白文本节点
            l && l.c(),// 如果 l 存在，则调用 l 的创建函数
                s = nt(),// 创建另一个空白文本节点
                v(e, "class", o = r[6] < r[0] ? "inline" : "hidden")// 设置 span 的 class
        }, m(c, a) {// 将创建的元素插入到 DOM 中
            U(c, e, a), w(e, t), U(c, i, a), l && l.m(c, a), U(c, s, a)
        }, p(c, a) {// 更新函数，用于处理数据变化
            a & 1 && o !== (o = c[6] < c[0] ? "inline" : "hidden") && v(e, "class", o), // 更新 class
                c[6] < c[0] ? l || (l = Sn(), l.c(), l.m(s.parentNode, s)) : l && (l.d(1), l = null)// 根据条件创建或销毁 l
        }, d(c) { // 销毁组件，清理相关状态
...
// 逻辑处理函数 jr，处理组件的初始化、属性设置和计时器
function jr(r, e, n) {
    let {text: t} = e, o = t.split(" "), i = 0;
    return setInterval(() => {// 设置计时器，定时更新 i 的值
        i < o.length && n(0, i++, i)
    }, 100), r.$$set = l => {// 定义组件属性设置函数
...
class Pr extends Se {
    constructor(e) {
        super(), // 调用父类构造函数
            $e(this, e, jr, Cr, xe, {text: 2})// 初始化组件
    }
}

// 数据包装函数 Ln，复制数组元素
function Ln(r, e, n) {
    const t = r.slice();// 创建 r 的副本
    return t[5] = e[n], t// 将 e 的第 n 个元素赋值给副本 t 的位置 5
}
...
function Tn(r) {
    ...
    function g() {
        return r[4](r[5])// 调用函数 r[4]，参数是 r[5]
    }

    return {
        c() {// 创建 DOM 元素
            e = E("li"),// 创建 li 元素
                n = E("button"),// 创建 button 元素
                t = Pt("svg"),// 创建 svg 元素
                o = Pt("path"),// 创建 path 元素
                i = Z(), // 创建空白节点
                l = we(s),// 设置文本内容
                c = Z(),// 创建另一个空白节点
                v(o, "stroke-linecap", "round"),// 设置 path 属性
                v(o, "stroke-linejoin", "round"),
                v(o, "d", "M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 01.865-.501 48.172 48.172 0 003.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0012 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z"),
                v(t, "class", "w-3.5 h-3.5"),// 设置 svg 的 class
                v(t, "fill", "none"),
                ...
                v(n, "class", "flex items-center text-left gap-x-3 py-2 px-3 text-sm text-slate-700 rounded-md hover:bg-gray-100 dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300")
        }, m(k, x) {// 将创建的元素插入到 DOM 中
            U(k, e, x), w(e, n), w(n, t), w(t, o), w(n, i), w(n, l), w(e, c),
            a || (d = je(n, "click", g), a = !0)// 为按钮添加点击事件监听
        }, p(k, x) {// 更新函数
            r = k, x & 8 && s !== (s = r[5].question + "") && Ue(l, s)// 如果问答内容改变则更新
        }, d(k) { // 销毁组件，清理相关状态
            k && G(e), a = !1, d()
        }
    }
...
function qr(r) {
    // 声明变量用于保存各种 UI 元素和状态
    let e, n, t, o, i, s, l, c, a, d, g, k, x, D, L, m,
        h = _e(r[3]), // 从 'r[3]' 提取数据（可能是一个数组），并初始化一个数组 'p' 来保存组件
        p = [];
    for (let _ = 0; _ < h.length; _ += 1) p[_] = Tn(Ln(r, h, _));// 循环遍历 'h'，为每个元素生成组件并将其添加到 'p' 中
    return {
        c() { // 创建组件的初始结构
            e = E("div"),// 主容器 div
                n = E("nav"),// 导航元素
                t = E("div"),// 用于品牌/Logo 的 div
                // 设置品牌/Logo 区域的内部 HTML
                t.innerHTML = '<img class="w-35 h-auto" src="assets/img.png" alt="Vanna Logo"/>\n' +
                    '<div class="lg:hidden">\n' +
                    '    <button type="button"\n' +
                ...
                l = E("li"),// 列表项元素
                c = E("button"),// 创建按钮元素
                // 设置按钮的内部 HTML(含 SVG 图标)
                c.innerHTML = `<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"
                ...
                a = Z(),// 另一个可能用于操作的元素
                d = E("li"),// 另一个列表项元素
                g = E("button"),// 创建另一个按钮元素
                // 设置按钮的内部 HTML（含 SVG 图标）
                g.innerHTML = `<svg class="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                            viewBox="0 0 16 16">
                ...
                新的提问`,
                k = Z();// 另一个可能用于操作的元素
            // 循环遍历 'p'，调用每个组件的 'c()' 方法以创建它们
            for (let _ = 0; _ < p.length; _ += 1) p[_].c();
            x = Z(),// 可能用于操作的元素
                D = E("div"),// 创建一个新的 div
                // 设置 div 的内部 HTML（含状态信息和注销链接）
                D.innerHTML = `<div class="py-2.5 px-7"><p class="inline-flex items-center gap-x-2 text-xs text-green-600"><span
                ...
                v(c, "class", "flex items-center gap-x-3 py-2 px-3 text-sm text-slate-700 rounded-md hover:bg-gray-100 dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300 border-t border-b border-gray-200 dark:border-gray-700 w-full"),
                v(g, "class", "flex items-center gap-x-3 py-2 px-3 text-sm text-slate-700 rounded-md hover:bg-gray-100 dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300"),
                ...
                v(e, "class", "hs-overlay hs-overlay-open:translate-x-0 -translate-x-full transition-all duration-300 transform hidden fixed top-0 left-0 bottom-0 z-[60] w-64 bg-white border-r border-gray-200 overflow-y-auto scrollbar-y lg:block lg:translate-x-0 lg:right-auto lg:bottom-0 dark:scrollbar-y dark:bg-slate-900 dark:border-gray-700")
        }, m(_, A) {// 挂载组件到 DOM
            U(_, e, A),// 使用 U 函数将主组件 'e' 挂载到 'r'（通常是 DOM 元素）上
                w(e, n),//将导航元素 'n' 挂载到主组件 'e' 上
                w(n, t),// 将品牌区域 't' 挂载到导航 'n' 上
                w(n, o), // 将一个操作元素 'o' 挂载到导航 'n' 上
                w(n, i),// 将另一个 div 元素 'i' 挂载到导航 'n' 上
                w(i, s),// 将无序列表 's' 挂载到 div 'i' 上
                w(s, l),// 将列表项 'l' 挂载到列表 's' 上
                w(l, c),// 将按钮 'c' 挂载到列表项 'l' 上
                w(s, a),// 将另一个操作元素 'a' 挂载到列表 's' 上
                w(s, d),// 将另一个列表项 'd' 挂载到列表 's' 上
                w(d, g),// 将按钮 'g' 挂载到列表项 'd' 上
                w(s, k);// 将最后一个操作元素 'k' 挂载到列表 's' 上
            // 循环遍历组件数组 'p'，挂载每个组件到列表 's' 中
            for (let P = 0; P < p.length; P += 1)
                p[P] && p[P].m(s, null);// 只挂载有效的组件
            // 将其他元素 'x' 和 'D' 挂载到导航 'n' 上
            w(n, x),
                w(n, D),
                // 检查标志 'L'，如果为 false，则添加事件监听器
            L || (m = [je(c, "click", function () {// 初始化事件监听器数组 'm'
                dt(r[0]) && r[0].apply(this, arguments)// 为按钮 'c' 添加点击事件
            }), je(g, "click", function () {// 为按钮 'g' 添加点击事件
                dt(r[1]) && r[1].apply(this, arguments) // 如果 r[1] 是有效的，调用它
            })], L = !0) // 设置 'L' 为 true，标记事件已添加
        }, p(_, [A]) {// 更新组件的函数
            if (r = _, A & 12) {// 检查条件，若 'A' 中包含特定标志（12）
                h = _e(r[3]);// 从 'r' 中提取数据并存储到 'h'
                let P;
                // 循环遍历 'h'，更新或添加组件
                for (P = 0; P < h.length; P += 1) {
                    const T = Ln(r, h, P);// 获取当前项的状态
                    // 如果当前组件已存在，调用其更新方法；否则创建新组件
                    p[P] ? p[P].p(T, A) : (p[P] = Tn(T), p[P].c(), p[P].m(s, null))
                }
                // 卸载多余的组件
                for (; P < p.length; P += 1) p[P].d(1);// 通过调用卸载方法来卸载组件
                p.length = h.length// 更新组件数组的长度
            }
        }, i: se,// 初始化的钩子函数
        o: se,// 卸载的钩子函数
        d(_) {// 卸载组件
            _ && G(e),// 如果需要卸载，则调用 G 函数处理主组件 'e'
                Je(p, _),// 卸载组件数组 'p'
                L = !1,// 重置标志 'L'
                ot(m) // 处理事件监听器数组 'm'
        }
...
function Ar(r, e, n) {
    // 解构赋值，提取传入参数 e 中的对象属性：getTrainingData、newQuestionPage、loadQuestionPage、questionHistory
    let {getTrainingData: t} = e, {newQuestionPage: o} = e, {loadQuestionPage: i} = e, {questionHistory: s} = e;
    // 定义函数 l，用于调用 loadQuestionPage 函数并传入指定问题的 ID
    const l = c => {
        i(c.id)
    };
    // 定义 $$set 方法，用于处理传入的参数更新
    // 该方法用于当父组件或外部组件传入新的 props 时，更新当前组件中的相应值
    return r.$$set = c => {
        // 如果传入对象中有 getTrainingData 属性，则更新 t
        "getTrainingData" in c && n(0, t = c.getTrainingData),
            // 如果传入对象中有 newQuestionPage 属性，则更新 o
        "newQuestionPage" in c && n(1, o = c.newQuestionPage),
            // 如果传入对象中有 loadQuestionPage 属性，则更新 i
        "loadQuestionPage" in c && n(2, i = c.loadQuestionPage),
            // 如果传入对象中有 questionHistory 属性，则更新 s
        "questionHistory" in c && n(3, s = c.questionHistory)
    }, [t, o, i, s, l]// 返回更新后的变量 t、o、i、s 以及函数 l
}

class Dr extends Se {
    // 构造函数，接收 e 作为参数，传递给父类构造函数
    constructor(e) {
        super(), // 调用父类构造函数，继承父类的方法和属性

            // 初始化组件，调用框架的 $e 函数，绑定当前组件实例的属性
            // 第一个参数是当前组件实例，第二个参数是传入的 props，第三个是组件的初始化函数 Ar
            // qr 是组件的渲染函数，xe 是框架内部使用的生命周期方法
            // 其中 getTrainingData、newQuestionPage、loadQuestionPage、questionHistory 是组件的 props
            $e(this, e, Ar, qr, xe, {
                getTrainingData: 0,
                newQuestionPage: 1,
                loadQuestionPage: 2,
...
// 立即执行函数，传入 r 和 e，r 是对象 Hr，e 是未使用的参数
(function (r, e) {
    // 在这里将 self 和 t 作为参数传入，定义了模块化的自执行函数
    (function (n, t) {
        // 将模块导出的内容赋值给 r.exports
        r.exports = t()
    })(self, function () {
        // 返回一个立即执行的函数，定义了一些工具函数
        return (() => {
            // 定义一个对象 n，存储模块相关的函数
            var n = {
                661: (s, l, c) => {
                    // 定义函数 a，用于判断数据类型，支持对 Symbol 的检测
                    function a(L) {
...
function Mr(r) {
    ...
    return {
        c() {
            e = E("div"), e.innerHTML = '<h1 class="text-3xl font-bold text-gray-800 sm:text-4xl dark:text-white">欢迎使用国能AI</h1> <p class="mt-3 text-gray-600 dark:text-gray-400">一个数据库助手</p>', v(e, "class", "max-w-4xl px-4 sm:px-6 lg:px-8 mx-auto text-center")
        }, m(n, t) {
            U(n, e, t)
        }, p: se, i: se, o: se, d(n) {
            n && G(e)
...
function zr(r) {
    ...
    return {
        c() {
            e = E("div"), n = E("input"), t = Z(), o = E("div"), i = E("div"), s = E("div"), s.innerHTML = "", l = Z(), c = E("div"), a = E("button"), a.innerHTML = '<svg class="h-3.5 w-3.5" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M15.964.686a.5.5 0 0 0-.65-.65L.767 5.855H.766l-.452.18a.5.5 0 0 0-.082.887l.41.26.001.002 4.995 3.178 3.178 4.995.002.002.26.41a.5.5 0 0 0 .886-.083l6-15Zm-1.833 1.89L6.637 10.07l-.215-.338a.5.5 0 0 0-.154-.154l-.338-.215 7.494-7.494 1.178-.471-.47 1.178Z"></path></svg>', v(n, "type", "text"), v(n, "class", "p-4 pb-12 block w-full bg-gray-100 border-gray-200 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-800 dark:border-gray-700 dark:text-gray-400"), v(n, "placeholder", "向我询问有关您的数据的问题，我可以将其转换为 SQL。"), v(s, "class", "flex items-center"), v(a, "type", "button"), v(a, "class", "inline-flex flex-shrink-0 justify-center items-center h-8 w-8 rounded-md text-white bg-blue-600 hover:bg-blue-500 focus:z-10 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"), v(c, "class", "flex items-center gap-x-1"), v(i, "class", "flex justify-between items-center"), v(o, "class", "absolute bottom-px inset-x-px p-2 rounded-b-md bg-gray-100 dark:bg-slate-800"), v(e, "class", "relative")
        }, m(k, x) {
...
function Jr(r) {
    ...
    return {
        c() {
            e = E("li"), n = E("img"), o = Z(), i = E("div"), c && c.c(), or(n.src, t = "/vanna.png") || v(n, "src", t), v(n, "class", "flex-shrink-0 w-[2.375rem] h-[2.375rem] "), v(n, "alt", "agent logo"), v(i, "class", "space-y-3 overflow-x-auto overflow-y-hidden"), v(e, "class", "max-w-4xl py-2 px-4 sm:px-6 lg:px-8 mx-auto flex gap-x-2 sm:gap-x-4")
        }, m(a, d) {
...
function eo(r) {
    ...
    return s = new pt({props: {$$slots: {default: [Xr]}, $$scope: {ctx: r}}}), {
        c() {
            e = E("li"), n = E("img"), o = Z(), i = E("div"), J(s.$$.fragment), or(n.src, t = "/vanna.png") || v(n, "src", t), v(n, "class", "flex-shrink-0 w-[2.375rem] h-[2.375rem] animate-bounce "), v(n, "alt", "agent logo"), v(i, "class", "space-y-3"), v(e, "class", "max-w-4xl py-2 px-4 sm:px-6 lg:px-8 mx-auto flex gap-x-2 sm:gap-x-4")
        }, m(c, a) {
...
function fi(r) {
    ...
    return {
        c() {
            Te && Te.c(), e = Z(), n = E("div"), t = E("div"), o = E("div"), i = E("div"), s = E("div"), l = E("div"), c = E("div"), c.innerHTML = '<h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200">Training Data</h2> <p class="text-sm text-gray-600 dark:text-gray-400">添加或删除训练数据。良好的训练数据是准确性的关键。</p>', a = Z(), d = E("div"), g = E("div"), k = E("button"), k.textContent = "View all", x = Z(), D = E("button"), D.innerHTML = `<svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M2.63452 7.50001L13.6345 7.5M8.13452 13V2" stroke="currentColor" stroke-width="2" stroke-linecap="round"></path></svg>
